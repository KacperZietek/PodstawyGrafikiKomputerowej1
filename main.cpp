...
public:
    ...
    virtual void render() override {
        ...
        // 4. Niebieskie tlo (symulacja flood fill)
        if (test_filled[1]) {
            // Rysujemy niebieskie tlo jako wielki prostokąt
            renderer.setColor(0, 0, 255);
            renderer.drawRectangle(0, 0, 1024, 768, true);
            // Ponownie rysujemy ksztalty na wierzchu
            if (test_filled[0]) {
                renderer.setColor(255, 255, 0);
                renderer.drawRectangle(100, 100, 200, 200, true);
                renderer.setColor(255, 255, 255);
        ...
        if (get_font()) {
            ...
            al_draw_text(get_font(), al_map_rgb(255,255,255), 10, 30, 0,
                "LPM - dodaj punkt | PPM - czysc | R - obrot | S - skalowanie | T - przesuniecie");
    ...
    virtual void on_key_press(int key) override {
        ...
        if (key == ALLEGRO_KEY_P)
            show_filled_polygon = !show_filled_polygon;

        // WYPEŁNIANIE OBSZARU
        if (key == ALLEGRO_KEY_1) {
            test_filled[0] = !test_filled[0];
            std::cout << "Boundary fill kwadratu: " << (test_filled[0] ? "WLACZONE" : "WYLACZONE") << std::endl;
        }

        if (key == ALLEGRO_KEY_2) {
            test_filled[1] = !test_filled[1];
            std::cout << "Flood fill tla: " << (test_filled[1] ? "WLACZONE" : "WYLACZONE") << std::endl;
        }

        if (key == ALLEGRO_KEY_3) {
            test_filled[2] = !test_filled[2];
            std::cout << "Boundary fill kola: " << (test_filled[2] ? "WLACZONE" : "WYLACZONE") << std::endl;
        }

        if (key == ALLEGRO_KEY_4) {
            test_filled[3] = !test_filled[3];
            std::cout << "Boundary fill trojkata " << (test_filled[3] ? "WLACZONE" : "WYLACZONE") << std::endl;
        }

        if (key == ALLEGRO_KEY_5) {
            // Reset wszystkich wypelnien
            for (int i = 0; i < 4; i++) {
                test_filled[i] = false;
            }
            std::cout << "Reset wszystkich wypelnien" << std::endl;
        }
